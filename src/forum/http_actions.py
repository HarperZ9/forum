...
class HttpActionMixin:
    """The mutating (POST) endpoint handlers.

    A mixin on HttpSurface: it reaches the orchestrator through ``self._orch`` and
    the validation helpers (``_read_json``, ``_str_field``, ``_context_budget``,
    ``_non_negative_int_field``) through ``self`` from HttpReadMixin. Never
