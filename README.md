...
- **Current floor:** 1.12.0 routes fresh per-task context from a peer brain, keeps a 28-lane default roster including `project-telos` and `model-foundry`, attaches deterministic human route frames to routing output, uses those frames to guide synthesis and select the default expert delivery profile for submit runs, supports optional task-level done criteria and per-wave submit checkpoints, derives run-room next actions, and preserves every route, task, result, and verification step in the ledger.
...
- **"A route should set the room, not just name the worker."** Route outputs now include a deterministic `forum.route-frame/v1` contract: domain, intent, posture, delivery profile, proof/domain lane hints, human-facing answer contract, and matched signals. It is local architecture, not model judgment, and route ledger entries witness it for later supervision data.
...
- `forum.route_frame`: the human route contract. It wraps a route with deterministic local fields for domain, intent, posture, delivery profile, proof/domain lane hints, a short answer contract, and the matched signals behind the frame. CLI, HTTP, MCP, and `Orchestrator.assign` route ledger entries expose the same `forum.route-frame/v1` payload, so routing becomes both dispatch data and communication posture.
