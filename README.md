...
- **Operator surface:** `forum status --json`, `forum doctor --json`, `forum demo --json`, `forum submit --json`, and `forum mcp` expose the Project Telos action envelope, submit receipts, actionable run-room snapshots, deterministic operator briefs, and native MCP tools including `forum.route`, `forum.prose.humanize`, `forum.status`, `forum.doctor`, `forum.ledger.summary`, and `forum.run.room`. The same CLI is available from source checkouts with `python -m forum`. The status payload also advertises shared CLI/MCP/plugin/IDE/TUI/app contracts for enterprise, research, creative, scientific, and education workflows.
