...
- `forum.lanes`: the proof-lane gate. A closed, frozen vocabulary of five proof lanes (observe, execute, validate, synthesize, verify), each granting an explicit scope set. `witness_route` refuses a route that names a lane outside the vocabulary or claims a scope its lane does not grant (over-routing), with a typed reason, and the rejection lands in the ledger as a first-class `lane_rejection` entry, never a silent drop. The decision keys off vocabulary membership and scope arithmetic, not the route author's wording, so a route cannot certify itself.
