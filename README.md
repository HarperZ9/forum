...
- **"A route should set the room, not just name the worker."** Route outputs now include a deterministic `forum.route-frame/v1` contract: domain, intent, posture, delivery profile, selected runtime tier/executor, proof/domain lane hints, human-facing answer contract, and matched signals. It is local architecture, not model judgment, and route ledger entries witness it for later supervision data.
- **"Delivery should follow the room the route set."** Submit passes the route frame's human contract into final-answer synthesis, then checks the delivered answer against the selected expert profile. Receipts include the witnessed route frame and the selected delivery profile source (`route_frame` or `explicit`), so the answer's posture is visible in the same record as the model, ledger range, and verification verdict.
...
- `forum.route_frame`: the human route contract. It wraps a route with deterministic local fields for domain, intent, posture, delivery profile, selected model tier/executor, proof/domain lane hints, a short answer contract, and the matched signals behind the frame. CLI, HTTP, MCP, and `Orchestrator.assign` route ledger entries expose the same `forum.route-frame/v1` payload, so routing becomes dispatch data, local runtime policy, and communication posture.
...
- `forum.mcp_surface`: the same tools over MCP (JSON-RPC on stdio), including `forum.runtime.inspect` and `forum.context.preflight`. It is a thin adapter over the HTTP surface, so the two can never drift.
