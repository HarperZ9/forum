...
- **Operator surface:** `forum status --json`, `forum doctor --json`, `forum demo --json`, `forum submit --json`, and `forum mcp` expose the Project Telos action envelope, submit receipts, actionable run-room snapshots, and native MCP tools including `forum.route`, `forum.prose.humanize`, `forum.status`, `forum.doctor`, `forum.ledger.summary`, and `forum.run.room`. The same CLI is available from source checkouts with `python -m forum`. The status payload also advertises shared CLI/MCP/plugin/IDE/TUI/app contracts for enterprise, research, creative, scientific, and education workflows.
- **Current floor:** 1.12.0 routes fresh per-task context from a peer brain, keeps a 28-lane default roster including `project-telos` and `model-foundry`, attaches deterministic human route frames and local runtime policy to routing output, uses those frames to guide synthesis and select the default expert delivery profile for submit runs, supports optional task-level done criteria and per-wave submit checkpoints, derives run-room next actions, and preserves every route, task, result, and verification step in the ledger.
...
- `forum.run_room`: the operator room read model. It projects the latest request's ledger slice into `forum.run-room/v1`: request, route frame, plan, task contracts, latest results, verdicts, checkpoints, answer, quality signals, and deterministic `next_actions`. CLI, HTTP, and MCP expose the same payload so platform tools can inspect a run without parsing raw replay entries.
