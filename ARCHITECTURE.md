...
pulls a tighter version and accepts it only if it is genuinely shorter and still covers
the request (the same coverage check the intent floor uses), so a tightening never costs
meaning; a revision that fails either test, or a reviser that crashes, is recorded and
discarded, and the original stands. The floor is the peer of the intent floor and the
