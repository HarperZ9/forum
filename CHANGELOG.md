...
- **Cheap-first**: the free deterministic floor runs every time; the model judge fires only on a flag, so the common covered case spends nothing. A flagged-but-paraphrased answer is cleared (ok true); a genuinely off-target answer is confirmed (ok false).
...
Pure standard library core; the judge is a model call behind the one executor seam. 193 tests, plus 2 gated real-model tests. Run in `examples/run_intent_judge.py`.
